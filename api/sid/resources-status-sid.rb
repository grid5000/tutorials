...
Restfully::Session.new(
  ...
  sites.each do |site|
    ...
    grid.sites[site[:name]].status(:query => { :reservations_limit => '100'})['nodes'].each do |node|
      site[:all_nodes] << node
      site[:dead_nodes] << node   if node[1]['hard'] == 'dead'
      site[:avail_nodes] << node  if filter_reservations(node[1]['reservations'], start_time, stop_time).empty? && node[1]['hard'] != 'dead'
    end
